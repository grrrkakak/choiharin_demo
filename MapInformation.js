...
const NearbyPlaces = () => {
    ...
    const fetchNearbyPlaces = async () => {
      try {
        ...
        const response = await fetch(
          `https://dapi.kakao.com/v2/local/search/keyword.json?x=${curLongitude}&y=${curLatitude}&radius=50&query=편의점`, // radius : 반경 조절, query = 카테고리
          {
            headers: {
  ...
  useEffect(() => {
    const fetchNearbyPlaces = async () => {
      try {
        ...
        // Kakao Map API로부터 주변 장소 정보를 요청
        const response = await fetch(
          `https://dapi.kakao.com/v2/local/search/keyword.json?x=${curLongitude}&y=${curLatitude}&radius=50&query=음식점`, // radius : 반경 조절, query = 카테고리
          {
            headers: {
              Authorization: 'KakaoAK 0b34fba760a465783a7d1649a9989945', // 'kakaoAk 본인API키'
            },
  ...
  useEffect(() => {
    const fetchNearbyPlaces = async () => {
      try {
        ...
        // Kakao Map API로부터 주변 장소 정보를 요청
        const response = await fetch(
          `https://dapi.kakao.com/v2/local/search/keyword.json?x=${curLongitude}&y=${curLatitude}&radius=50&query=마트`, // radius : 반경 조절, query = 카테고리
          {
            headers: {
              Authorization: 'KakaoAK 0b34fba760a465783a7d1649a9989945', // 'kakaoAk 본인API키'
            },
  ...
  return (
    <div className="BackTemplate" style={{marginBottom: '0px'}}>
      <div className="App-title"><VscCompass></VscCompass>  위치 탐색</div>
      {location && (
        <Map
          style={{
            width: '512px',
      ...
      <div className='BackTemplate' style={{marginTop:'0px'}}>
        <div className='App-title' style={{fontSize:'1rem', height:'1.5rem', marginBottom:'0px'}}>장소 선택</div>
        <ul className='ListDesign'>
        {places_1.map((place) => (
          <li key={place.id} onClick={() => handlePlaceSelect(place)} className='ElementDesign'>
